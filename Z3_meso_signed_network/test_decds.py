...
def Qs_txt(txt, cluster):
    ...
    if one_net_edge[-1] == '':
        one_net_edge = one_net_edge[:-1]
    else:
        pass

    a, b, s = one_net_edge[0].split("\t")  # $$改动31/11；36 V
    if int(a) == 1:
        for i in range(len(one_net_edge)):
            a, b, s = one_net_edge[i].split("\t")   # $$改动31/16:30
            array[int(a)-1][int(b)-1] = float(s)  # $$改动31/16:18 int -> float
            array[int(b)-1][int(a)-1] = float(s)  # $$改动31/16:18 int -> float
    else:
        for i in range(len(one_net_edge)):
            a, b, s = one_net_edge[i].split("\t")
            array[int(a)][int(b)] = float(s)  # $$改动31/16:18 int -> float
            array[int(b)][int(a)] = float(s)  # $$改动31/16:18 int -> float
...
def main(signed_file, no_signed_file):
    signedTxt = signed_file  # TODO 带符号的网络 路径
    noSignedTxt = no_signed_file  # TODO 不带符号的网络 路径
...
#     file_name = '2_RD'
#     signed_file = base_dir + '/data/signed/' + file_name + '.txt'  # 符号网络路径
#     no_signed_file = base_dir + '/data/no_signed/' + file_name + '.txt'  # 无符号网络路径
#     qs = main(signed_file, no_signed_file)
...
if __name__ == '__main__':
    ...
    p_null = []
    for i in range(50):
        signed_file = base_dir + '/data/signed/' + file_name + null_name + str(i) + '.txt'
        no_signed_file = base_dir + '/data/no_signed/' + file_name + null_name + str(i) + '.txt'
        qs = main(signed_file, no_signed_file)
        p_null.append(qs)
